...
export default function NavTab({ setOnClick, tabType }) {
  return (
    <div className={styles.nav__container}>
      <div className={styles.nav__container__press} onClick={setOnClick}>
        <div
          className={`${styles['nav__press-contents']} ${tabType.subscribe === 'ALL_PRESS' ? styles['contents-select'] : ''}`}
          data-view-type="all-press-tab"
        >
          전체 언론사
        </div>
        <div
          className={`${styles['nav__press-contents']}  ${tabType.subscribe === 'SUBSCRIBED_PRESS' ? styles['contents-select'] : ''}`}
          data-view-type="my-press-tab"
        >
          내가 구독한 언론사
        </div>
      </div>
      <div className={styles.nav__container__view_type} onClick={setOnClick}>
        <div id="list-tab" data-view-type="list-view-tab">
          <svg width={30} height={30} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              className={`${tabType.view === 'LIST_VIEW_TYPE' ? styles['grid-option-select'] : styles['grid-option']}`}
        ...
        <div id="grid-tab" data-view-type="grid-view-tab">
          <svg width={30} height={30} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              className={`${tabType.view === 'GRID_VIEW_TYPE' ? styles['grid-option-select'] : styles['grid-option']}`}
